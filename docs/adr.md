...
## ADR-001: Uso de Minimal API com .NET 8

**Contexto:** Projeto acadêmico TicketPrime, foco em simplicidade e agilidade.

**Decisão:** Adotar Minimal API em vez de Controllers MVC tradicionais.

**Consequências:**
- ✅ Menos boilerplate (`Program.cs` único vs Controllers + Startups)
- ✅ Roteamento inline facilita entendimento do fluxo completo
- ⚠️ Perde organização por separação de concerns (mitigado com Services/Repositories separados)

---

## ADR-002: Dapper como Micro-ORM

**Contexto:** Necessário acesso a banco de dados SQL Server com consultas parametrizadas anti-SQL Injection.

**Decisão:** Usar Dapper em vez de Entity Framework Core.

**Consequências:**
- ✅ SQL explícito permite otimização fina de queries
- ✅ Performance superior (mapeamento direto DataReader → objeto)
- ⚠️ Mais código manual (INSERT/SELECT escritos à mão)
...
**Contexto:** Necessário autenticação stateless entre frontend Blazor Server e API.

**Decisão:** Token JWT com claims de CPF e Perfil (ADMIN/USER).

**Consequências:**
- ✅ Sem estado no servidor (escalável)
- ✅ Claims embutidas no token (CPF, Perfil)
- ⚠️ Revogação manual não é possível sem blacklist

---

## ADR-004: Separação em Camadas (Service + Repository)

**Contexto:** Regras de negócio AV2 (R1-R4) precisam ser testáveis isoladamente.
...
**Contexto:** Regra de negócio R4: desconto só se aplica se `PrecoPadrao >= ValorMinimoRegra`.
...
- ⚠️ Desconto não é valor fixo, mas percentual

---

## ADR-006: INNER JOIN para GET /api/reservas/{cpf}

**Contexto:** AV2 exige endpoint único que retorne dados consolidados de reserva + evento + usuário.
...
- ⚠️ DTO precisa ser mantido em sincronia com a query SQL

---

## ADR-007: Limite de 2 Reservas por CPF por Evento (R2)

**Contexto:** Regra de negócio R2 impede que um CPF reserve mais que 2 ingressos para o mesmo evento.
...
- ⚠️ Race condition teórica em requisições simultâneas (aceitável para projeto acadêmico)
